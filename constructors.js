...
Movie.prototype.changeRating = function(rating) {
  return (this.rating = (rating + this.rating) / 2);
};
...
const savedPosts = function savedPosts(id, title, rating) {
  savedPosts.id = id;
  savedPosts.title = title;
  savedPosts.rating = rating;
};
...
User.prototype.removeSavedPost = function(id) {
  this.savedPosts = this.savedPosts.filter(element => {
...
User.prototype.changePostRating = function(id, newRating) {
  this.savedPosts.filter(element => {
    if (element.id === id) {
